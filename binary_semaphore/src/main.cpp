...
void initial(){
  ...
  countingSema = xSemaphoreCreateCounting(2,0);
...
void task2(void *pvPara){
  while(1){
    xSemaphoreTake(countingSema,portMAX_DELAY); 
    for(int i = 0; i < 5; i++){
    NEO();
    }
    xSemaphoreTake(countingSema,portMAX_DELAY);
...
void setup()
{
  ...
  xTaskCreate(&task2,"NEO LED",150,NULL,1,NULL);
